	...
	<body>
		<?php
			if($_POST){
				...
				$myfile=fopen('hasil/input.txt','w')or die("tidak bisa dibuka");
				$txt="";
				for($i=0;$i<count($dtfilling);$i++){
					$txt .= '
<div class="form-group">
<label class="label-control" for="'.$dtfilling[$i].'">'.$dtheading[$i].'</label><input type="text" name="'.$dtfilling[$i].'" class="form-control" placeholder="'.$dtheading[$i].'" />
</div>';
				}		
